...
const {
  WebSocketServer
} = require("protoo-server");
const mediasoup = require("mediasoup");
const ConfRoom = require("./lib/Room");
const config = require('./config.js');
//link .env file to this file
require('dotenv').config();

(async () => {
  var privateKey = process.env.NODE_ENV === 'localrun' ?
    fs.readFileSync(__dirname + config.local_SslKey, 'utf8') :
    fs.readFileSync(__dirname + config.sslKey, 'utf8');

  var certificate = process.env.NODE_ENV === 'localrun' ?
    fs.readFileSync(__dirname + config.local_SslCert, 'utf8') :
    fs.readFileSync(__dirname + config.sslCert, 'utf8');
